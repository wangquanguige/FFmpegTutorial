...
static AVCodecID audio_codec_id;
...
static int32_t decode_packet(bool flushing) {
    ...
    while(result >= 0) {
        ...
        if(result == AVERROR(EAGAIN) || result == AVERROR_EOF) {
            ...
            std::cerr << "Error: failed to receive frame, result: " << result << std::endl;
            return -1;
        }
        ...
        std::cout << "frame->nb_samples: " << frame->nb_samples << ", frame->channels: " << frame->channels << std::endl;
    }
...
int32_t get_audio_format(AVCodecContext *codec_ctx) {
    ...
    if(av_sample_fmt_is_planar(sfmt)) {
        ...
        std::cout << "Warning: the sample format the decoder produced is planar channel only." << std::endl;
        sfmt = av_get_packed_sample_fmt(sfmt);
    }

    int n_channles = codec_ctx->channels;
    if((ret = get_format_from_sample_fmt(&fmt, sfmt)) < 0) {
        return -1;
    }

    std::cout << "Play command: ffplay -f " << std::string(fmt) << " -ac " << n_channles << " -ar " << codec_ctx->sample_rate << " output.pcm" <<std::endl;
    return 0;
}
