...
static int32_t init_input_video(char *video_input_file, const char *video_format) {
    ...
    if(!video_input_format) {
        std::cerr << "Error: failed to find proper AVInputFormat for format: " << std::string(video_format) << std::endl;
        return -1;
    }
...
static int32_t init_output(char *output_file) {
    ...
    // 这里其实不合规矩，主要是r_frame_rate是50/1，avg_frame_rate是25/1，导致封装后两个都变成50/1
    video_fmt_ctx->streams[in_video_st_idx]->r_frame_rate = (AVRational){STREAM_FRAME_RATE, 1};
    ...
    if(!(fmt->flags & AVFMT_NOFILE)) {
        ...
        if(result < 0) {
            std::cerr << "Error: avio_open output file failed!" << std::string(output_file) << std::endl;
            return -1;
        }
    }
...
int32_t init_muxer(char *video_input_file, char *audio_input_file, char *output_file) {
    ...
    result = init_input_audio(audio_input_file, "aac");
    if(result < 0) {
        return result;
    }
    ...
    while(1) {
        if(av_compare_ts(cur_video_pts, in_video_st->time_base, cur_audio_pts, in_audio_st->time_base) <= 0) {
            ...
            if(pkt.pts == AV_NOPTS_VALUE) {
                ...
                pkt.dts = pkt.dts;
                std::cout << "frame_duration: " << frame_duration << ", pkt.duration: " << pkt.duration << ", pkt.pts " << pkt.pts << std::endl;
            }
        ...
        std::cout << "Final pts: " << pkt.pts << ", duration: " << pkt.duration << ", output_stream->time_base: " 
                    << output_stream->time_base.num << "/" << output_stream->time_base.den << std::endl;
